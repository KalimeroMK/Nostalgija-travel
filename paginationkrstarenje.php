...
$query = "SELECT COUNT(*) as num FROM tekst WHERE type='Крстарење'";
...
// Get page data
$query1 = "SELECT * FROM tekst WHERE type='Крстарење' ORDER BY id DESC LIMIT $start, $limit";
...
if ($lastpage > 1) {
    ...
    if ($page > 1) {
        $paginate .= '<li><a href="/krstarenje/' . $prev . '/" aria-label="Previous"><i class=" glyphicon glyphicon-chevron-left"></i></a></li>';
    } else {
    ...
    if ($lastpage < 7 + ($stages * 2)) {
        ...
        for ($counter = 1; $counter <= $lastpage; $counter++) {
            if ($counter == $page) {
                $paginate .= '<li class="active"><a href="/krstarenje/' . $counter . '/">' . $counter . '</a></li>';
            } else {
                $paginate .= '<li><a href="/krstarenje/' . $counter . '/">' . $counter . '</a></li>';
            }
        }
        ...
        elseif ($lastpage - ($stages * 2) > $page && $page > ($stages * 2)) {
            ...
            $paginate .= "<li><a href='#'>...</a></li>";
            $paginate .= '<li><a href="/krstarenje/' . $LastPagem1 . '/">' . $LastPagem1 . '</a></li>';
            $paginate .= '<li><a href="/krstarenje/' . $lastpage . '/">' . $lastpage . '</a></li>';
    ...
    // Next
    if ($page < $counter - 1) {
        $paginate .= '<li><a href="/krstarenje/' . $next . '/" aria-label="&laquo;"><i class=" glyphicon glyphicon-chevron-right"></i></a></li>';
    } else {
