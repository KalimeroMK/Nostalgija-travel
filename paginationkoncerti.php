...
$query = "SELECT COUNT(*) as num FROM tekst WHERE type='Концерти'";
...
// Get page data
$query1 = "SELECT * FROM tekst WHERE type='Концерти' ORDER BY id DESC LIMIT $start, $limit";
...
$paginate .= '<nav class="text-center">';
    $paginate .= '<ul class="pagination">';
        ...
        if ($page > 1) {
        $paginate .= '<li><a href="/koncerti/' . $prev . '/" aria-label="Previous"><i class=" glyphicon glyphicon-chevron-left"></i></a></li>';
        } else {
        ...
        if ($counter == $page) {
        $paginate .= '<li class="active"><a href="/koncerti/' . $counter . '/">' . $counter . '</a></li>';
        } else {
        $paginate .= '<li><a href="/koncerti/' . $counter . '/">' . $counter . '</a></li>';
        }
        }
        ...
        $paginate .= "<li><a href='#'>...</a></li>";
        $paginate .= '<li><a href="/koncerti/' . $LastPagem1 . '/">' . $LastPagem1 . '</a></li>';
        $paginate .= '<li><a href="/koncerti/' . $lastpage . '/">' . $lastpage . '</a></li>';
        ...
        // Next
        if ($page < $counter - 1) {
        $paginate .= '<li><a href="/koncerti/' . $next . '/" aria-label="&laquo;"><i class=" glyphicon glyphicon-chevron-right"></i></a></li>';
        } else {
