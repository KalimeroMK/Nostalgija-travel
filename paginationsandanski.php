...
$query = "SELECT COUNT(*) as num FROM tekst WHERE grad ='Сандански'";
...
// Get page data
$query1 = "SELECT * FROM tekst WHERE grad ='Сандански' ORDER BY id DESC LIMIT $start, $limit";
...
if ($lastpage > 1) {
    ...
    if ($page > 1) {
        $paginate .= '<li><a href="/sandanski/' . $prev . '/" aria-label="Previous"><i class=" glyphicon glyphicon-chevron-left"></i></a></li>';
    } else {
    ...
    if ($lastpage < 7 + ($stages * 2)) {
        ...
        for ($counter = 1; $counter <= $lastpage; $counter++) {
            if ($counter == $page) {
                $paginate .= '<li class="active"><a href="/sandanski/' . $counter . '/">' . $counter . '</a></li>';
            } else {
                $paginate .= '<li><a href="/sandanski/' . $counter . '/">' . $counter . '</a></li>';
            }
        }
        ...
        elseif ($lastpage - ($stages * 2) > $page && $page > ($stages * 2)) {
            ...
            $paginate .= "<li><a href='#'>...</a></li>";
            $paginate .= '<li><a href="/sandanski/' . $LastPagem1 . '/">' . $LastPagem1 . '</a></li>';
            $paginate .= '<li><a href="/sandanski/' . $lastpage . '/">' . $lastpage . '</a></li>';
    ...
    // Next
    if ($page < $counter - 1) {
        $paginate .= '<li><a href="/sandanski/' . $next . '/" aria-label="&laquo;"><i class=" glyphicon glyphicon-chevron-right"></i></a></li>';
    } else {
